...
import {setPrice} from '../../actions/camera';
...
const CheckPanel = ({
  ...
  const [topAnim, setTopAnim] = useState(
    new Animated.Value<number>(windowHeight),
  );
  const [position, setPosition] = useState(Position.BOTTOM);
  const storePrice = (price: any) => dispatch(setPrice(price));
  const [priceByUser, setPriceByUser] = useState<Map<string, number>>(Map());
  const [itemsByUser, setItemsByUser] = useState<Map<string, List<Item>>>(
    Map(),
  );
  ...
  const panelHeight = useSelector(
    (state: RootState) => state.check.panelHeight,
  );
  ...
  useEffect(() => {
    ...
    for (const item of items) {
      const count: number = usersItemsList
        .filter((_item: {itemUid: any}) => _item.itemUid === item.uid)
        .count();
      if (count === 0) continue;
      let price =
        items.find((_item: {uid: any}) => _item.uid === item.uid)!.price /
        count;
      pricesByItems = pricesByItems.set(item.uid, price);
    }
    let itemsCountByUsers: Map<string, number> = Map();
    items.map((item: {uid: string}) => {
      const count = usersItemsList
        .filter((el: {itemUid: any}) => el.itemUid === item.uid)
        .count();
      if (count === 0) return;
      itemsCountByUsers = itemsCountByUsers.set(item.uid, count);
    });
    for (const user of users) {
      const unsortedItemsByUser: List<string> = usersItemsList
        .filter((item: {userUid: any}) => item.userUid === user.uid)
        .map((item: {itemUid: any}) => item.itemUid);
      let sortedItemsByUser: List<string> = List();
      items.forEach((item: {uid: string}) => {
      ...
      splitByUser = splitByUser.set(
        user.uid,
        sortedItemsByUser.map(uid => {
          const _item = items.find((item: {uid: string}) => item.uid === uid)!;
          const splitCount = (
            _item.count / itemsCountByUsers.get(uid)!
          ).toFixed(2);
          const splitPrice = pricesByItems.get(uid)!;
          const item = {...items.find((i: {uid: string}) => i.uid === uid)!};
  ...
  const deleteUserDialog = (userUid: string) => {
    Alert.alert(
      f({id: 'dialog.delete.title'}),
      f({id: 'dialog.delete.text'}),
      [
        {
          text: f({id: 'dialog.delete.yes'}),
          onPress: () => deleteUserHandler(userUid),
        },
        {
          text: f({id: 'dialog.delete.no'}),
  ...
  const renderUsers = () => {
    return users.map(
      (user: {uid: string | number | null | undefined; index: number}) => {
        const price = priceByUser.has(user.uid)
          ? priceByUser.get(user.uid) || null
          : null;
        return (
          <TouchableOpacity
            key={user.uid}
  ...
  const getTotalPrice = (): string => {
    return items
      .reduce((result: any, object: {price: any}) => result + object.price, 0)
      .toFixed(2);
  };
  ...
  const renderSectionHeader = ({
    ...
    const outerColor =
      section.index > 0 ? usersColor.getColor(section.index - 1) : 'white';
    const innerColor = usersColor.getColor(section.index);
    return (
      <TouchableWithoutFeedback
        style={{backgroundColor: outerColor}}
        onPress={sectionClick}>
  ...
  const renderSectionFooter = ({
    ...
    const color = usersColor.getColor(section.index);
    return <View style={[styles.sectionFooter, {backgroundColor: color}]} />;
  };

  const sections: Section[] = [];
  users.forEach((user: {uid: string; index: any}) => {
  ...
  const renderLostPrice = () => {
    ...
    items.forEach((item: {uid: any}) => {
      const exists = usersItemsList.find(
        (el: {itemUid: any}) => el.itemUid === item.uid,
      );
      if (!exists) allItemsDistributed = false;
    });
    if (allItemsDistributed) return null;
    const usersPrice = users.reduce(
      (total: number, user: {uid: string}) =>
        total +
        (priceByUser.has(user.uid) ? priceByUser.get(user.uid) || 0 : 0),
      0,
    );
    const diff = parseFloat(getTotalPrice()) - usersPrice;
    setPrice(parseInt(diff.toFixed(2)));
  ...
  return (
    <Animated.View
      style={[styles.panel, {top: topAnim, elevation: 10, opacity: opacity}]}
